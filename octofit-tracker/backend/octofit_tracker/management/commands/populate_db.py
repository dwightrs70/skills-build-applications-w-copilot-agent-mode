...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        # Create teams
        # Ensure members field is initialized as an empty list at the time of object creation
        team1 = Team(_id=ObjectId(), name='Blue Team', members=[])
        team2 = Team(_id=ObjectId(), name='Gold Team', members=[])
        team1.save()
        team2.save()

        # Add user IDs to the members field
        team1.members = [str(user._id) for user in users]
        team1.save()

        team2.members = [str(user._id) for user in users]
        team2.save()
